...
class SequenceEncoding:
    ...
    def get_ProtVec_encoding_with_original(self, ProtVec, seq, overlap=True):
        if overlap:
            encodings = []
    ...
    def get_ProtVec_encoding(self, ProtVec, seq, overlap=True):
        if overlap:
            encodings = []
            for i in range(len(seq)-2):
                encodings.append(ProtVec[seq[i:i+3]]) if ProtVec.__contains__(seq[i:i+3]) else encodings.append(ProtVec["<unk>"])
        else:
            encodings_1, encodings_2, encodings_3 = [], [], []
            for i in range(0, len(seq), 3):
                if i+3 <= len(seq):
                    encodings_1.append(ProtVec[seq[i:i+3]]) if ProtVec.__contains__(seq[i:i+3]) else encodings_1.append(ProtVec["<unk>"])
                if i+4 <= len(seq):
                    encodings_2.append(ProtVec[seq[i+1:i+4]]) if ProtVec.__contains__(seq[i+1:i+4]) else encodings_2.append(ProtVec["<unk>"])
                if i+5 <= len(seq):
                    encodings_3.append(ProtVec[seq[i+2:i+5]]) if ProtVec.__contains__(seq[i+2:i+5]) else encodings_3.append(ProtVec["<unk>"])
                
            encodings = [encodings_1, encodings_2, encodings_3]
        return encodings
        
    def get_encoding_with_original(self, seq, overlap=True):
        seq = seq.upper()
    ...
    def get_encoding(self, seq, overlap=True):
        ...
        if self.encoding_type == "ProtVec":            
            ...
            for res in seq:
                if res not in SequenceEncoding.residue_types:
                    res = "X"
                encoding_data.append(np.array(encoding[res]))
