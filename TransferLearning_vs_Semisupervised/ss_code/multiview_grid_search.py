...
def main(enc1, enc2, enc1_X_train, enc2_X_train, enc1_X_test, enc2_X_test, y_train, y_test, labeled_percentage, model, results_folder):
    ...
    for k in range(n_splits):
        ...
        try:
            ...
            print(f"enc2_X_train.shape: {enc2_X_train.shape}")
        ...
        ct_y_proba = ct.predict(enc1_X_test, X2=enc2_X_test)
        ...
        print(f"\tExperiment with {enc1} and {enc2} using {labeled_percentage*100}% labeled instances (k={k}) took {time.strftime('%Hh %Mm %Ss', time.gmtime(time.time() - start))} | Current time: {time.strftime('%Hh %Mm %Ss', time.gmtime())}", flush=True)
